...
contract EventImplementationTicketsTest is Test {
    ...
    function createTestEvent() internal returns (uint256) {
        ...
        tags[0] = "event";
        tags[1] = "test";
        eventImpl.createEvent("Test Event", 1 ether, 100, startTime, endTime, "uri", LibStorage.EventType.Paid, EVENT_EXPENSES, "Test", "Local", tags);
        vm.stopPrank();
        return 0;
    }
    function testBuyTicket_HappyPath() public {
        uint256 eventId = createTestEvent();
        vm.startPrank(user);
        paymentToken.approve(address(eventImpl), 1 ether);
        eventImpl.buyTicket(eventId);
        vm.stopPrank();
        address ticketOwner = eventImpl.getTicketOwner(0);
        assertEq(ticketOwner, user);
        uint256 ticketEvent = eventImpl.getTicketEvent(0);
        assertEq(ticketEvent, eventId);
    }
}
